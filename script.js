...
// --- 以下就是您需要的“鼠标跟随爱心”功能代码 ---
// 它独立于任何按钮，只要鼠标移动就会触发
document.addEventListener('mousemove', (e) => {
    // 使用 Math.random() 来控制爱心生成的频率，避免过于密集
    if (Math.random() > 0.95) { // 表示大约只有5%的鼠标移动事件会创建爱心
        const heart = document.createElement('div');
        heart.classList.add('floating-heart'); // 您可以为这个类定义特定的CSS样式
        heart.textContent = '💖'; // 您可以改成 '❤️' 或其他喜欢的表情
        
        // 设置爱心的初始位置在鼠标指针处
        heart.style.left = `${e.clientX}px`;
        heart.style.top = `${e.clientY}px`;
        
        // 设置初始大小，不干扰CSS动画中的translateY变换
